...
# using the import-statement to get access functions from different packages
...
print('The scalar product of X and Y is: ' + str(np.dot(X,Y)))
# initialize Z as array of ones with size (1,5)
Z =np.ones((1,5))
# multiply Z by 3
print('multiply Z by 3: ' +str(Z*3))
...
print('The matrix product of M and O is: ' + str(np.dot(M,O)))
# multiply M with Y
...
dax = np.genfromtxt('time_series_dax_2022.csv',delimiter=';',skip_header=1,usecols=3)
...
plt.plot(dax)
plt.xlabel('Time')
plt.ylabel('Low points of dax index')
plt.show()
